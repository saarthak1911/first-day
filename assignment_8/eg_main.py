...
@tool
def knowledge(txt_input):
    """
    this knowledge function is a knowledge lookup tool which has great general knowledge.      
    
    :param txt_input : str input as plain text
    :returns txt_input result as str
    """
    try:
        result = llm.invoke(txt_input)
        return str(result)
    except:
        return "Error : Can't Answer this Question"
...
while True:
    ...
    print("(Tool Called :",llm_output["tool-calls"]["name"]")") # it will not work 
    print("\n\n", result["messages"]